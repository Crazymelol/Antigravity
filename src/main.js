...
import ShopScene from '/src/ShopScene.js';
import EconomyManager from '/src/EconomyManager.js';
...
window.onload = function () {
    ...
    // Workaround: We'll show the overlay on top. When a mode is picked, we restart the scene with data.
    ...
    tournaments.forEach(t => {
        ...
        row.onclick = () => {
            if (eco.enterTournament(t.entry)) {
                // Matchmaking Sim
                simulateMatchmaking(overlay, () => {
                    overlay.remove(); // Remove lobby
